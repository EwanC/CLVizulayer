...
// CHECK-NEXT: node_2[label="clEnqueueFillBuffer"];
...
int main() {
  ...
  cl_uint Pattern = 42;
  Ret = clEnqueueFillBuffer(State.OutOfOrderQueue, State.BufferA, &Pattern,
                            sizeof(Pattern), 0, State.AllocSize, 1, &Events[0],
                            &Events[1]);
  CHECK(Ret);
