...
// CHECK-NEXT: node_0[label="clCommandCopyImageKHR
// CHECK-NEXT: src_image = 0x{{[0-9a-fA-F]+}}
// CHECK-NEXT: dst_image = 0x{{[0-9a-fA-F]+}}
// CHECK-NEXT: src_origin = {1, 1, 0}
// CHECK-NEXT: dst_origin = {1, 1, 0}
...
int main() {
  ...
  Ret = State.clCommandCopyImageKHR(command_buffer, nullptr, nullptr,
                                    State.ImageA, State.ImageB, Origin, Origin,
